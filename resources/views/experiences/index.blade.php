...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
        <a href="{{ route('experiences.create') }}" class="button text-white bg-theme-1 shadow-md mr-2">Add New Project</a>
        <div class="dropdown">
    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
        <table class="table table-report -mt-2">
            <thead>
                <tr>
                    <th class="whitespace-no-wrap">DETAIL</th>
                    <th class="whitespace-no-wrap">CERTIFICATE</th>
                    <th class="text-center whitespace-no-wrap">START WORK</th>
                    <th class="text-center whitespace-no-wrap">END WORK</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="intro-x">
                    <td>
                        <div class="font-medium whitespace-no-wrap">{{ $experience->company }}</div>
                        <div class="text-gray-600 text-xs whitespace-no-wrap">{{ $experience->job_desc }}</div>
                    </td>
                    <td><a href="{{ $experience->certificate }}" target="_blank">{{ $experience->certificate }}</a></td>
                    <td class="text-center">{{ $experience->start_work }}</td>
                    <td class="text-center">{{ $experience->end_work }}</td>
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3" href="{{ route('experiences.edit', $experience->id) }}"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
    ...
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-no-wrap items-center">
        <ul class="pagination">
            {!! $experiences->appends(['search' => request()->query('search')])->links() !!}
        </ul>
    </div>
    <!-- END: Pagination -->
</div>
