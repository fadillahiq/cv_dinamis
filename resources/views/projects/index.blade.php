...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
        <a href="{{ route('projects.create') }}" class="button text-white bg-theme-1 shadow-md mr-2">Add New Project</a>
        <div class="dropdown">
                    ...
                    <td class="w-40">
                        <div class="flex">
                            <div class="w-10 h-10 image-fit zoom-in">
                                <img alt="Midone Tailwind HTML Admin Template" class="tooltip rounded-full" src="{{ Storage::url($project->image) }}" title="Uploaded at {{ $project->updated_at->diffForHumans() }}">
                            </div>
                        </div>
                    </td>
                    <td>
                        <a href="#" class="font-medium whitespace-no-wrap">{{ $project->title }}</a>
                        <div class="text-gray-600 text-xs whitespace-no-wrap">{{ $project->desc }}</div>
                    </td>
                    <td class="text-center">{{ $project->link }}</td>
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3" href="{{ route('projects.edit', $project->id) }}"> <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit </a>
